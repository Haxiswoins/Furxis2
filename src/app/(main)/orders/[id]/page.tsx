...
import { getOrderById, reinstateOrder } from '@/lib/data-service';
...
export default function OrderDetailPage() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [orderData, siteContent] = await Promise.all([
  ...
  const handleCancelClick = () => {
    if (order.status === '申请中' || order.status === '待确认') {
  ...
  const renderCancelButton = () => {
    if (order.status === '申请中' || order.status === '待确认' || order.status === '排队中') {
      return (
