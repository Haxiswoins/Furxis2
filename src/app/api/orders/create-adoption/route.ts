...
const charactersFilePath = path.join(process.cwd(), 'data', 'characters.json');
...
async function readData(filePath: string): Promise<any> {
  try {
    ...
    if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
      // For arrays, return empty array. For objects, you might need specific defaults.
      if (filePath.endsWith('s.json')) return [];
      return {};
    }
    throw error;
  }
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Increment applicant count on the character
    const characters = await readData(charactersFilePath) as Character[];
    const charIndex = characters.findIndex(c => c.id === character.id);
    if (charIndex > -1) {
        characters[charIndex].applicants += 1;
        await writeData(charactersFilePath, characters);
    }
